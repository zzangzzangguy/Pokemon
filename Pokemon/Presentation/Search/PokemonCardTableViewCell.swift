    ...
    func configure(with card: PokemonCard, isFavorite: Bool) {
        ...
        favoriteButton.rx.tap
            .subscribe(onNext: { [unowned self] _ in
                let newState = !self.favoriteButton.isSelected
                self.favoriteButton.isSelected = newState
                RealmManager.shared.updateFavorite(for: self.cardInfo?.id ?? "", isFavorite: newState)
